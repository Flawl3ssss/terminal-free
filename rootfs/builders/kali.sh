...
sudo chroot "$ROOTFS" /bin/bash -c "dpkg --force-all --configure -a" || true
...
sudo chroot "$ROOTFS" /bin/bash -c "apt-get install -y --no-install-recommends --allow-unauthenticated --force-yes \
  ...
  procps vim less sudo" || true
sudo chroot "$ROOTFS" /bin/bash -c "dpkg --force-all --configure -a" || true
sudo chroot "$ROOTFS" /bin/bash -c "apt-get remove --purge -y systemd systemd-sysv systemd-timesyncd systemd-resolved dbus dbus-user-session || true" || true
sudo chroot "$ROOTFS" /bin/bash -c "dpkg --force-all --configure -a" || true
