...
class SettingsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val bgSwitch = findViewById<SwitchCompat>(R.id.bg_mode_switch)
        bgSwitch.isChecked = prefs.getBoolean("keep_alive", false)
        bgSwitch.setOnCheckedChangeListener { _, isChecked ->
            prefs.edit { putBoolean("keep_alive", isChecked) }
            TerminalService.ensure(this)
        }

        val killSwitch = findViewById<SwitchCompat>(R.id.kill_on_close_switch)
        killSwitch.isChecked = prefs.getBoolean("kill_on_close", true)
        killSwitch.setOnCheckedChangeListener { _, isChecked ->
            prefs.edit { putBoolean("kill_on_close", isChecked) }
        }
        ...
        findViewById<TextView>(R.id.export_config_btn).setOnClickListener {
            try {
                ...
                val fileName = "terminal-free_config.json"
        ...
        findViewById<TextView>(R.id.import_config_btn).setOnClickListener {
            try {
                val candidates = listOf(
                    java.io.File(android.os.Environment.getExternalStorageDirectory(), "TerminalFree/terminal-free_config.json"),
                    java.io.File(getExternalFilesDir(null), "terminal-free_config.json")
                )
                val file = candidates.firstOrNull { it.exists() }
                if (file == null) {
                    Toast.makeText(this, "No terminal-free_config.json found", Toast.LENGTH_LONG).show()
    ...
    private fun backupDir(): java.io.File {
        val shared = java.io.File(
            android.os.Environment.getExternalStorageDirectory(), "TerminalFree"
        )
        shared.mkdirs()
    ...
    private fun backupFiles(): List<java.io.File> {
        ...
        java.io.File(android.os.Environment.getExternalStorageDirectory(), "TerminalFree")
        ...
        Thread {
            try {
                ...
                if (!java.io.File(rootfsDir, "etc/os-release").exists() &&
                    ...
                    throw RuntimeException("Backup does not look like a Terminal Free backup")
                }
